...
function register_clients_cpt() {
    $labels = array(
        'name'                  => _x( 'Clientes', 'Post Type General Name', 'alejandro-template' ),
        'singular_name'         => _x( 'Cliente', 'Post Type Singular Name', 'alejandro-template' ),
        'menu_name'             => __( 'Clientes', 'alejandro-template' ),
        'name_admin_bar'        => __( 'Cliente', 'alejandro-template' ),
        'archives'              => __( 'Archivo de Clientes', 'alejandro-template' ),
        'attributes'            => __( 'Atributos de Cliente', 'alejandro-template' ),
        'parent_item_colon'     => __( 'Cliente Padre:', 'alejandro-template' ),
        'all_items'             => __( 'Todos los Clientes', 'alejandro-template' ),
        'add_new_item'          => __( 'Añadir Nuevo Cliente', 'alejandro-template' ),
        'add_new'               => __( 'Añadir Nuevo', 'alejandro-template' ),
        'new_item'              => __( 'Nuevo Cliente', 'alejandro-template' ),
        'edit_item'             => __( 'Editar Cliente', 'alejandro-template' ),
        'update_item'           => __( 'Actualizar Cliente', 'alejandro-template' ),
        'view_item'             => __( 'Ver Cliente', 'alejandro-template' ),
        'view_items'            => __( 'Ver Clientes', 'alejandro-template' ),
        'search_items'          => __( 'Buscar Cliente', 'alejandro-template' ),
        'not_found'             => __( 'No se encontraron clientes', 'alejandro-template' ),
        'not_found_in_trash'    => __( 'No se encontraron clientes en la papelera', 'alejandro-template' ),
        'featured_image'        => __( 'Logo del Cliente', 'alejandro-template' ),
        'set_featured_image'    => __( 'Establecer logo del cliente', 'alejandro-template' ),
        'remove_featured_image' => __( 'Eliminar logo del cliente', 'alejandro-template' ),
        'use_featured_image'    => __( 'Usar como logo del cliente', 'alejandro-template' ),
        'insert_into_item'      => __( 'Insertar en el cliente', 'alejandro-template' ),
        'uploaded_to_this_item' => __( 'Subido a este cliente', 'alejandro-template' ),
        'items_list'            => __( 'Lista de clientes', 'alejandro-template' ),
        'items_list_navigation' => __( 'Navegación de lista de clientes', 'alejandro-template' ),
        'filter_items_list'     => __( 'Filtrar lista de clientes', 'alejandro-template' ),
    );
    $args = array(
        'label'                 => __( 'Cliente', 'alejandro-template' ),
        'description'           => __( 'Gestión de clientes de freelancer', 'alejandro-template' ),
        'labels'                => $labels,
...
function add_clients_meta_boxes() {
    add_meta_box(
        'client_details',
        __( 'Detalles del Cliente', 'alejandro-template' ),
...
function render_client_details_meta_box( $post ) {
    ...
    <div style="background: #f0f0f1; padding: 10px; border-bottom: 1px solid #ccd0d4; margin: -12px -12px 12px -12px; display: flex; justify-content: flex-end;">
        <button type="button" id="toggle-client-privacy" class="button">
            ...
            <span id="privacy-btn-text"><?php _e( 'Ocultar Datos', 'alejandro-template' ); ?></span>
        </button>
    </div>

    <table class="form-table" id="client-details-table">
        <tr class="client-info-row">
            <th><label for="client_website"><?php _e( 'Sitio Web', 'alejandro-template' ); ?></label></th>
            <td><input type="url" id="client_website" name="client_website" value="<?php echo esc_url( $website ); ?>" class="regular-text"></td>
        </tr>
        <tr class="client-info-row">
            <th><label for="client_email"><?php _e( 'Correo Electrónico', 'alejandro-template' ); ?></label></th>
            <td><input type="email" id="client_email" name="client_email" value="<?php echo esc_attr( $email ); ?>" class="regular-text"></td>
        </tr>
        <tr class="client-info-row">
            <th><label for="client_phone"><?php _e( 'Teléfono', 'alejandro-template' ); ?></label></th>
            <td><input type="text" id="client_phone" name="client_phone" value="<?php echo esc_attr( $phone ); ?>" class="regular-text"></td>
        </tr>
        <tr class="client-info-row">
            <th><label for="client_status"><?php _e( 'Estado del Proyecto', 'alejandro-template' ); ?></label></th>
            <td>
                <select id="client_status" name="client_status">
                    <option value="lead" <?php selected( $project_status, 'lead' ); ?>><?php _e( 'Potencial (Lead)', 'alejandro-template' ); ?></option>
                    <option value="active" <?php selected( $project_status, 'active' ); ?>><?php _e( 'Activo', 'alejandro-template' ); ?></option>
                    <option value="on_hold" <?php selected( $project_status, 'on_hold' ); ?>><?php _e( 'En pausa', 'alejandro-template' ); ?></option>
                    <option value="completed" <?php selected( $project_status, 'completed' ); ?>><?php _e( 'Completado', 'alejandro-template' ); ?></option>
                    <option value="cancelled" <?php selected( $project_status, 'cancelled' ); ?>><?php _e( 'Cancelado', 'alejandro-template' ); ?></option>
                </select>
            </td>
        </tr>
        <tr class="client-info-row">
            <th><label for="client_service"><?php _e( 'Servicio Contratado', 'alejandro-template' ); ?></label></th>
            <td>
                <select id="client_service" name="client_service">
                    <option value="diseno_web" <?php selected( $service, 'diseno_web' ); ?>><?php _e( 'Diseño Web', 'alejandro-template' ); ?></option>
                    <option value="desarrollo_web" <?php selected( $service, 'desarrollo_web' ); ?>><?php _e( 'Desarrollo Web', 'alejandro-template' ); ?></option>
                    <option value="seo_sem" <?php selected( $service, 'seo_sem' ); ?>><?php _e( 'SEO / SEM', 'alejandro-template' ); ?></option>
                    <option value="mantenimiento" <?php selected( $service, 'mantenimiento' ); ?>><?php _e( 'Mantenimiento', 'alejandro-template' ); ?></option>
                    <option value="consultoria" <?php selected( $service, 'consultoria' ); ?>><?php _e( 'Consultoría', 'alejandro-template' ); ?></option>
                    <option value="otros" <?php selected( $service, 'otros' ); ?>><?php _e( 'Otros', 'alejandro-template' ); ?></option>
                </select>
            </td>
        </tr>
        <tr class="client-info-row">
            <th><label for="client_budget"><?php _e( 'Presupuesto (€)', 'alejandro-template' ); ?></label></th>
            <td><input type="number" id="client_budget" name="client_budget" value="<?php echo esc_attr( $budget ); ?>" class="regular-text" step="0.01"></td>
        </tr>
        <tr class="client-info-row">
            <th><label for="client_start_date"><?php _e( 'Fecha de Inicio', 'alejandro-template' ); ?></label></th>
            <td><input type="date" id="client_start_date" name="client_start_date" value="<?php echo esc_attr( $start_date ); ?>" class="regular-text"></td>
        </tr>
        <tr class="client-info-row">
            <th><label for="client_considerations"><?php _e( 'Consideraciones', 'alejandro-template' ); ?></label></th>
    ...
    (function($) {
        ...
        $('#toggle-client-privacy').on('click', function() {
            ...
            if (!isHidden) {
                ...
                btnText.text('<?php _e( 'Mostrar Datos', 'alejandro-template' ); ?>');
                ...
                btnText.text('<?php _e( 'Ocultar Datos', 'alejandro-template' ); ?>');
...
function set_custom_edit_cliente_columns($columns) {
    $columns['client_service'] = __( 'Servicio', 'alejandro-template' );
    $columns['client_email'] = __( 'Email', 'alejandro-template' );
    $columns['client_status'] = __( 'Estado', 'alejandro-template' );
    $columns['client_budget'] = __( 'Presupuesto', 'alejandro-template' );
    return $columns;
}
